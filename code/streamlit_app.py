...
def show_main_page():
    ...
    with main_col2:
        ...
        if st.session_state.get("run_analysis", False):
            ...
            """
            # Prepare Dissolving Animation (Same setup but different animation logic)
            dissolving_html = """
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <style>
                        ...
                        #text-canvas { width: 800px; height: 200px; }
                            ...
                            for (let y = 0; y < h; y += step) {
                                for (let x = 0; x < w; x += step) {
                                    ...
                                    if (imageData[i + 3] > 128) {
                                        ...
                                        // Calculate outward direction for explosion
                                        const dx = x - cx;
                                        const dy = y - cy;
                                        const angle = Math.atan2(dy, dx);
                                        const dist = Math.sqrt(dx*dx + dy*dy);
                                        // Fast speed + some randomness
                                        const speed = 5 + Math.random() * 8 + (dist / 20);
            ...
            try:
                ...
                with loading_placeholder.container():
                     components.html(dissolving_html, height=370)
                
                # Wait for animation to finish visually (1.2 seconds)
                time.sleep(1.2)
